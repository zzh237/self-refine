...
def parse_ground_truth_math500_answer(answer_str: str, dataset_name: str) -> str:
    """
    Parses the ground truth string to extract the final clean answer.
    """
    ...
    if dataset_name in ['math_500']:
        match = re.search(r"\\boxed\{(.*?)\}", answer_str)
        if match:
            return match.group(1).replace(",", "").strip()
    
    # For QA datasets, the answer is already clean
...
def parse_ground_truth_gsm8k_answer(answer_str: str, dataset_name: str) -> str:
...
def evaluate_gsm8k(prediction: dict, ground_truth_item: dict,dataset_name: str) -> dict:
    """Evaluates a prediction for the gsm8k dataset."""
    pred_answer_str = str(prediction.get("answer", "")).strip()
    gold_answer_str = parse_ground_truth_gsm8k_answer(ground_truth_item.get("answer", ""),dataset_name)

    # Normalize prediction as well, in case it includes reasoning
    if "####" in pred_answer_str:
        pred_answer_str = parse_ground_truth_gsm8k_answer(pred_answer_str,dataset_name)
...
def evaluate_qa(prediction: dict, ground_truth_item: dict) -> dict:
    ...
    return {
        ...
        "accuracy": f1  # I will use f1 score
    }
...
def evaluate_prediction(prediction: dict, ground_truth_item: dict, dataset_name: str) -> dict:
    ...
    #     return {"error": 1}
    if not isinstance(prediction, dict) or "error" in str(prediction.get("answer", "")).lower():
    ...
    if dataset_name ==  'math_500':
        return evaluate_math(prediction, ground_truth_item, dataset_name)
    elif dataset_name == 'gsm8k':
        return evaluate_gsm8k(prediction, ground_truth_item, dataset_name)
